...
function fastek_update(){
  ...
  for($i = 1; $i <= count($fastek_sheet); $i++){
    ...
    $stock_get_b = $stock_get_b['quantity'] + $d_fa;
...
function ideac_update(){
  ...
  for($i = 1; $i <= count($ideac_sheet); $i++){
    ...
    $f_id = str_replace('"', " ", str_replace("/", " ", $ideac_sheet[$i]["F"])); // GDL
...
function importacion_update(){
  ...
  for($i = 1; $i <= count($importacion_sheet); $i++){
    $a_im = str_replace('"', " ", str_replace("/", " ", $importacion_sheet[$i]["A"])); // Referencia
    $b_im = str_replace('"', " ", str_replace("/", " ", $importacion_sheet[$i]["B"])); // Nombre de producto
    ...
    $e_im_tigf = $e_im_tig / $EXTERNAL_CURRENCY;
    $conslt_pe = $db->query('SELECT * FROM ps_product WHERE reference ="'.$a_im.'"');
    $conslt_pe = $conslt_pe->fetch_array(MYSQLI_ASSOC);
...
function techsmart_update(){
  ...
  for($i = 1; $i <= count($techsmart->item)-1; $i++){
    ...
    $stock_get_e = $db->query('SELECT * FROM ps_product WHERE reference = "'.$a_te.'"');
